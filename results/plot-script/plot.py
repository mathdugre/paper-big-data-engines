...
def bar(*, col_names, benchmark_dir, experiment, parameters, xlabel, save_name=None, ylim=None, **kwargs):
    ...
    xticks_label = sorted({float(x.replace(freedom+"=", "")) for k in filenames for x in k.split("/")[-2].split(":")[2:] if freedom in x})
    ...
    for i, (framework, experiments) in enumerate(results.items()):
        data = [
            ...
            for key in sorted(experiments, key=lambda k: float(re.search(f"{freedom}=(\d+\.?\d*)", k).group(1)))
        ]
...
def gantt(df, xlabel="Time [s]", *, pre_process, group, x_limit, save_name, framework, ylabel, title):
